...
std::string HTTPServer::construct_file_path(const std::string &file_name)
{
...
bool HTTPServer::is_method_supported(const std::string &method)
{
  return method == "GET";
}

void HTTPServer::log_response()
{
  std::cout << "Logging response ..." << std::endl;
  std::cout << response->get_response() << std::endl;
}

std::tuple<std::string, std::string> HTTPServer::get_mime_type_and_extension(const std::string &file_name)
{
...
bool HTTPServer::add_file_to_body(const std::string &file_path, const std::string &file_extension)
{
